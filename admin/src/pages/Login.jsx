    ...
    try {
      ...
      if (state === 'Admin') {
       ...
       const  {data} = await axios.post(backendUrl + '/api/admin/login', {email,password})
        if (data.success) {
          localStorage.removeItem('dToken'); // 👈 Clear doctor token
          localStorage.setItem('aToken', data.token); // ✅ Set admin token
          setAToken(data.token); // ✅ Context-based setter for admin token (or however you're managing it)
